...
$ip = shell_exec("../__DEV__/./devices.sh");

$ips = explode(" ", $ip);
echo "<select name='ip_addr' id='select'>";

   foreach ($ips as $device)
     {
      
       if (isset($_POST['ip_addr'])) 
          {
       echo "<option>{$_POST['ip_addr']}</option>"; 
            }
...
<?php
 ...
 if (isset($_POST['submit'])) 
  ...
  ob_implicit_flush(true);
   ...
   echo "<input type='button' value='STOP ATTACK' id='button_stop'  onclick=window.location.href='automatic.php'>";
